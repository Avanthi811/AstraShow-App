...
const Dashboard = () => {
  ...
  return ! loading ? (
    <>
     ...
     <div className='relative flex flex-wrap gap-4 mt-6'>
        ...
        <div className='flex flex-wrap gap-4 w-full'>
            {dashboardCards.map((card,index)=>(
                <div key={index} className='flex items-center justify-between px-4 py-3 bg-blue-400/10 border border-blue-500/40 rounded-md max-w-50  w-full'>
                        <div>
     ...
     <div className='relative flex flex-wrap gap-6 ml-1 mt-4 max-w-5xl'>
        <BlurCircle top='100px' left='-10%' />
            {dashboardData.activeShows.map((show)=>(
                <div key={show._id} className='w-55 rounded-lg overflow-hidden h-full pb-3 bg-blue-400/10 border border-blue-500/40 hover:-translate-y-1 transition duration-300'>
